...
class IPTVSetupImpl:
	...
	elif self.openSSLVersion == '.1.1':
            self.getOpenssl21Finished()
    ...
    ###################################################
    def getOpenssl21Finished(self):
        printDBG("IPTVSetupImpl.getOpenssl2Finished")
        # we detect new version OpenSSL without symbol OPENSSL_1.0.0
        self.openSSLVersion = '.1.1'
        self.libSSLPath = ""
        ...
        for ver in ['.1.0.0', '.1.0.2', '.1.1']:
            for path in ['/usr/lib/', '/lib/', '/usr/local/lib/', '/local/lib/']:
                for library in ['libssl.so.1.1', 'libcrypto.so.1.1']:
                    library += ver
                    fullLibraryPath = os_path.join(path, library)
                    if os_path.isfile(fullLibraryPath):
                        openSSlVerMap[library] =  fullLibraryPath
        ...
        linksTab = []
        for library in ['libssl.so', 'libcrypto.so']:
            if (library + '.1.0.0') not in openSSlVerMap and (library +'.1.1') in openSSlVerMap:
                linksTab.append([openSSlVerMap[library + '.1.1'], openSSlVerMap[library + '.1.1'][:-1] + '0'])
