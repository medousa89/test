# Embedded file name: /usr/lib/enigma2/python/Plugins/Extensions/IPTVPlayer/hosts/hostguardaserie.py
...
def gettytul():
    return 'https://www.guardaserie.productions/'


class GuardaSerieClick(CBaseHostClass):

    def __init__(self):
        CBaseHostClass.__init__(self, {'history': 'guardaserie.productions',
         'cookie': 'guardaserie.productions.cookie'})
        self.USER_AGENT = 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/73.0.3683.103 Safari/537.36'
        self.MAIN_URL = 'https://www.guardaserie.productions/'
        self.HEADER = {'User-Agent': self.USER_AGENT,
         'Accept': 'text/html',
         'Accept-Encoding': 'gzip',
         'Referer': self.MAIN_URL}
        self.AJAX_HEADER = MergeDicts(self.HEADER, {'X-Requested-With': 'XMLHttpRequest',
    ...
    def getPageCF(self, baseUrl, params = {}, post_data = None):
        ...
        params['cloudflare_params'] = {'domain': 'guardaserie.productions',
    ...
    def listMainMenu(self, cItem):
        ...
        params = MergeDicts(self.defaultParams, {'user-agent': self.USER_AGENT,
         'referer': self.MAIN_URL,
         'accept-encoding': 'gzip',
         'accept': 'text/html'})
        ...
        for sData in data:
            ...
            for item in data:
                ...
                params.update({'type': 'video',
                 'good_for_fav': False,
                 'title': title,
                 'urls': reObj.findall(item),
                 'url': cItem['url'] + '?s={0}&e={1}'.format(season, episode),
                 'desc': desc,
    ...
    def getLinksForVideo(self, cItem):
        ...
        for item in cItem['urls']:
            urlsTab.append({'name': self.cm.getBaseUrl(item, True),
